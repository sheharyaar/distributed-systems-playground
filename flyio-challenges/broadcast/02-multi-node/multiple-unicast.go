...
func main() {
	...
	n.Handle("broadcast", func(m maelstrom.Message) error {
		...
		if err := n.Reply(m, reply); err != nil {
			return err
		}
	...
	// this is looped unicast, this cannot ensure serializability
	n.Handle("nodecast", func(m maelstrom.Message) error {
		var body map[string]any
		...
		val, ok := body["message"].(float64)
		if !ok {
			return errors.New("invalid value in nodecast message")
		}
